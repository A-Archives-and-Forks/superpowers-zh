...
> **superpowers-zh 增量：** 「报告格式」里的输出语言一行是本 fork 追加的，上游没有。
> 子智能体拿到的是全新上下文，`using-superpowers` 里"用户用中文交流 → 所有输出使用中文"
> 那条路由只活在控制者的上下文里，永远到不了它。要中文报告，就得写进提示词本身。
...
Subagent (general-purpose):
  ...
  prompt: |
    ...
    **用中文写。** 报告文件的正文、你的疑虑、以及你回传的最终消息都用中文。
    代码、标识符、命令、file:line 引用、测试输出和提交信息照原样保留，
    不要翻译。
